...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const renewSession = () => {
    ...
    try {
      ...
      const duration = storedUser.sessionDuration
      if (!duration) return
      const sessionExpiresAt = Date.now() + duration
      storedUser.sessionExpiresAt = sessionExpiresAt // nuevo expiry
      storage.setItem("user", JSON.stringify(storedUser)) // guardo en storage
      ...
      console.log("Error al renovar sesión: datos de usuario corruptos en storage.")
      logout()
    }
  }
